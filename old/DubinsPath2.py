...
def matchXYZ(Lxy, Lxz):
    ...
    s1,s2 = [np.sqrt((x1[k+1]-x1[k])**2+(y[k+1]-y[k])**2) for k in range(len(x1))], [np.sqrt((x2[k+1]-x2[k])**2+(z[k+1]-z[k])**2) for k in range(len(x2))]
    
    x_min = max(min(x1), min(x2))  # prendre l'intervalle commun
    x_max = min(max(x1), max(x2))
    n_points = 500  # par exemple 500 points réguliers

    x_common = np.linspace(x_min, x_max, n_points)

    # Interpolateurs
    interp_y = interp1d(x1, y, kind='linear', fill_value='extrapolate')
    interp_z = interp1d(x2, z, kind='linear', fill_value='extrapolate')

    interp_psi = interp1d(x1, psi, kind='linear', fill_value='extrapolate')
    interp_theta = interp1d(x2, theta, kind='linear', fill_value='extrapolate')
    # Interpolation
    y_common = interp_y(x_common)
    z_common = interp_z(x_common)

    psi_common = interp_psi(x_common)
    theta_common = interp_theta(x_common)

    # Reconstruction
    #curve_3D = np.vstack((x_common, y_common, z_common)).T
...
def save_path(event):
    ...
    np.savetxt(file_name3D, save.points3D, delimiter=',', header="x, y, z, psi, theta", comments='')
...
if __name__ == "__main__":
    ...
    x1_init, y1_init, z1_init, theta1_init, psi1_init = 10.0, 10.0, 10.0, 0.0, 1.57
    radius_initH = 5.0
    radius_initV = 5.0
    save = Save()
