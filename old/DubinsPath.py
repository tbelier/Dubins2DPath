...
def addS(L):
    x0,y0,angle0 = L[0]
    Lnew, sk = [[x0,y0,angle0,0.0]],0.0
    for k in range(1,len(L)):
        xk,yk, anglek = L[k]
        xk_1, yk_1, anglek_1 = L[k-1]
        sk += np.sqrt((xk-xk_1)**2 + (yk-yk_1)**2)
        Lnew.append([xk,yk,anglek,sk])
    #print(Lnew)
    return Lnew

def find_Lindex(L1,L2):
    L1 = addS(L1)
    L2 = addS(L2)
    sMax_L1 = L1[-1][3]
    sMax_L2 = L2[-1][3]
    ...
    for k1 in range(len(L1)):
        if L1[k1][3]/sMax_L1 == 0.0: 
            L_index.append(0)
            continue
            
        for k2 in range(1,len(L2)):
            if L2[k2-1][3]/sMax_L2 <= L1[k1][3]/sMax_L1 <= L2[k2][3]/sMax_L2:
                L_index.append(k2)
                continue
...
def matchXYZ(Lxy, Lxz):
    if len(Lxy) < len(Lxz): Lxz = Lxz[find_Lindex(Lxy,Lxz)]
    if len(Lxy) > len(Lxz): Lxy = Lxy[find_Lindex(Lxz,Lxy)]
    ...
    #print(len(y))
    #print(len(z))
    Lxyz = np.column_stack((x, y, z, theta, psi))
